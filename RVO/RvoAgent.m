classdef RvoAgent
    ...
    methods (Access = public)
        function obj = RvoAgent(x0,y0,xg,yg,iAgent)
        ...
        function [heading, Velocity] = calcaction(obj)
            if isempty(obj.neighbourIds) % No neighbours to worry about, go full speed at the goal.
                ...
                for iNeighbour = 1:length(obj.neighbourIds)
                    ...
                    [VO_j] = obj.define2DReciprocalVelocityObstacle(p_i,v_i,r_i,p_j,v_j,r_j,tau_j);
...
%                 Velocity = obj.matchvelocity(Velocity);
        ...
        function [RVO] = define2DReciprocalVelocityObstacle(obj,p_i,v_i,r_i,p_j,v_j,r_j,tau)
            p_a = [p_i';0]; v_a = [v_i';0];
            p_b = [p_j';0]; v_b = [v_j';0];
            p_i = p_a;
            p_j = p_b;
            ...
            v_j = v_b;
             ...
             % OBSTACLE KNOWLEDGE
            lambda_ab = p_b - p_a;
            r_c = r_i + r_j + 4;        % Configuration radius with a small tolerance offset
...
%           leadingTest = sign((Bx - Ax) * (Y - Ay) - (By - Ay) * (X - Ax))
            ...
            % /////////////////////////////////////////////////////////////
            % IS Va INSIDE THE VO
            [RVO.isVaInsideCone] = obj.isInCone(RVO,v_a); 
